...
deseq2.additive<-function(obj_exp,outfolder,invname,pop){
	design2<-obj_exp[[1]]
	table.fil<-obj_exp[[2]]
	...
	if (length(levels(design2$condition))>1){
		...
		if (go.w.deseq==1){
			...
			pvalues<-vector()
				...
				design.f<-design2
				design.f$condition<-as.numeric(factor(design.f$condition))
